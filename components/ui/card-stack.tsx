      ...
      >
        ...
        {canGoPrev && (
          <button
            ...
            className="absolute left-2 md:left-8 top-1/2 -translate-y-1/2 z-[110] flex h-12 w-12 items-center justify-center rounded-full border border-black/10 bg-white/80 dark:bg-zinc-800/80 dark:border-white/10 text-foreground shadow-lg backdrop-blur-md transition-all duration-200 hover:bg-white dark:hover:bg-zinc-700 hover:scale-110 active:scale-95 focus:outline-none cursor-pointer"
        ...
        {canGoNext && (
          <button
            ...
            className="absolute right-2 md:right-8 top-1/2 -translate-y-1/2 z-[110] flex h-12 w-12 items-center justify-center rounded-full border border-black/10 bg-white/80 dark:bg-zinc-800/80 dark:border-white/10 text-foreground shadow-lg backdrop-blur-md transition-all duration-200 hover:bg-white dark:hover:bg-zinc-700 hover:scale-110 active:scale-95 focus:outline-none cursor-pointer"
...
function DefaultFanCard({ item, active }: { item: CardStackItem; active: boolean }) {
  ...
  React.useEffect(() => {
    ...
    if (isHovered) {
      videoRef.current.play().catch((err) => {
        // Silently handle autoplay interruption (e.g. user hasn't interacted with page yet)
  ...
  return (
    ...
    >
        ...
        {item.imageSrc ? (
          <img
            ...
            className={cn(
              ...
              isHovered && item.videoSrc ? "opacity-0" : "opacity-100"
            )}
            draggable={false}
            loading="eager"
          />
      ...
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent z-20" />

      {/* content */}
      <div className="relative z-30 flex h-full flex-col justify-end p-5 select-none pointer-events-none">
        <div className="truncate text-lg font-semibold text-white drop-shadow-md">
          {item.title}
        </div>
        {item.description ? (
          <div className="mt-1 line-clamp-2 text-sm text-white/90 drop-shadow-sm">
            {item.description}
          </div>
        ) : null}
      </div>
